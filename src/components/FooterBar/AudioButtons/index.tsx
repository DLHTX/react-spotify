...
import { IMusicStore, MODE } from '../../../store/interface/IMusicStore'
...
const AudioButtons: React.FC<IProps> = ({ MusicStore }) => {
  ...
  const handleRandomPlay = () => {
    if (MusicStore?.state.playMode !== MODE.SHUFFLE_PLAYBACK) {
      MusicStore?.SET_PLAYMODE(MODE.SHUFFLE_PLAYBACK)
      message.success('已开启随机播放')
    } else {
      MusicStore?.SET_PLAYMODE(MODE.PLAY_IN_ORDER)
      message.success('已关闭随机播放')
    }
  }

  const handleLoopPlay = () => {
    if (MusicStore?.state.playMode !== MODE.SINGLE_CYCLE) {
      MusicStore?.SET_PLAYMODE(MODE.SINGLE_CYCLE)
      message.success('已开启单曲循环')
    } else {
      MusicStore?.SET_PLAYMODE(MODE.PLAY_IN_ORDER)
      message.success('已关闭单曲循环')
    }
  }
  ...
  return (
    <div className={'flex text-white items-center justify-center'}>
      {/* 随机播放 */}
      <div className="relative" onClick={handleRandomPlay}>
        <svg
          style={{
            fill:
              MusicStore?.state.playMode === MODE.SHUFFLE_PLAYBACK
                ? 'rgb(29, 185, 84)'
                : '',
          }}
      ...
      {/* 单曲循环 */}
      <div className="relative" onClick={handleLoopPlay}>
        <svg
          style={{
            fill:
              MusicStore?.state.playMode === MODE.SINGLE_CYCLE
                ? 'rgb(29, 185, 84)'
                : '',
          }}
          className={styles.svg}
